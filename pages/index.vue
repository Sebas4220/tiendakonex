    ...
    <header class="sticky-top shadow-sm">
      ...
      <nav class="navbar bg-black text-white px-3 py-3 border-bottom border-dark">
        ...
        >
          ...
          <div class="d-flex flex-column lh-1" style="min-width: 120px">
            ...
            <div class="text-center mt-2" style="white-space: nowrap">
              <small class="text-white-50" style="font-size: 0.7rem">
                💵 Ref:
                <strong class="text-warning">{{ tasaCambio.toFixed(2) }} VES</strong>
              </small>
            </div>-->
          </div>
          ...
          <!-- Acciones de la Derecha (Sin Usuario) -->
      ...
      <!-- 2. SUBMENÚ DE CATEGORÍAS (ABRE VISTA DEDICADA) -->
      <div
      ...
      >
        <div class="container-fluid px-3" style="font-size: 0.8rem">
          ...
          >
            <!-- Botón de Inicio (Muestra todo en la pantalla principal) -->

            <!-- Categorías Dinámicas que abren la Vista Dedicada -->
...
const mensajePendiente = ref(null); // Almacena temporalmente el pedido o consulta antes de abrir la pestaña

// --- ESTADO PARA EL SWITCH DE MONEDA ---
const monedaActiva = ref("USD"); // Puede ser 'USD' o 'VES'

// Tasa de cambio definida de manera reactiva
  ...
  banners: [
    {
      ...
      imagen:
        "https://images.unsplash.com/photo-1563453392212-326f5e854473?auto=format&fit=crop&w=300&q=80",
    },
    {
      id: 2,
...
const iniciarRedireccionSoporte = (textoPersonalizado = null) => {
  ...
  mensajePendiente.value = textoPersonalizado; // Si es null, el HTML sabrá que es soporte genérico
...
const ejecutarRedireccionWhatsApp = () => {
  ...
  if (mensajePendiente.value) {
    ...
    // Si era una consulta limpia desde la cabecera
...
// Mantener por compatibilidad si se llama directo de manera inmediata en otro punto
const abrirSoporte = () => {
  let numeroTelefono = "584245542026";
...
onMounted(() => {
  if (typeof window !== "undefined" && window.localStorage) {
    const favoritosGuardados = JSON.parse(localStorage.getItem("mis_favoritos") || "[]");

    if (favoritosGuardados.length > 0) {
      datos.value.productos.forEach((prod) => {
        if (favoritosGuardados.includes(prod.id)) {
...
const enviarAWhatsApp = () => {
  ...
  carritoStore.itemsProcesados.forEach((item) => {
    ...
    // Si el nombre pasa de 25 caracteres, lo corta y pone "..."
    const nombreCorto =
      item.nombre.length > 25 ? item.nombre.slice(0, 25) + "..." : item.nombre;

    // Se agrega el ID único para control interno de inventario usando el nombre recortado
    texto += `• [ID: ${item.id}] x${item.cantidad} ${nombreCorto} ${tipoPrecio}\n`;
  ...
  // Lanzamos el modal visual pasando la cadena estructurada del pedido
